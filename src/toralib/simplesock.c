...
int sendbuf(int sockobj, char *sendbuffer, uint32_t buflen, short timeout_sec){
	...
	log_info("Buffer %s of size %d sent with retval:%d",sendbuffer,buflen,sent);
	return sent;
}

int recvbuf(int sockobj, char *recvbuffer, uint32_t buflen, short timeout_sec){
...
int fixed_recvbuf(int sockobj, char *recvbuffer, uint32_t buflen, short timeout_sec){
	//recv via the socket connection
	...
	log_info("Buffer %s of size %d recv with retval:%d %d",recvbuffer,buflen,brecv,ptrindex);
