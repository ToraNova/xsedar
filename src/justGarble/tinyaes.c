...
{
  uint32_t dest[128];
  unsigned char* blob = (unsigned char*) state;
  uint32_t bloblen = 16;
...
static uint8_t xtime(uint8_t x)
{
  ...
  /*uint8_t test = x;
  printf("XTIME IN: %" PRIu8 "\n", x);*/
  uint8_t out = ((x<<1) ^ (((x>>7) & 1) * 0x1b));
 /* printf("XTIME OUT: %" PRIu8 "\n", out);

  uint8_t x_dash_one = test << 1;
  printf("x << 1 %" PRIu8 "\n", x_dash_one);

  uint8_t x_dash_seven_and_one = (test >> 7) & 1;
  printf("(x>>7 &1) %" PRIu8 "\n", x_dash_seven_and_one);

  uint8_t xored = x_dash_seven_and_one * 0x1b;
  printf("* 1b : %" PRIu8 "\n", xored);

  uint8_t one_b = (xored ^ x_dash_one);
  printf("XOR %" PRIu8 "\n", one_b);
*/
  return out;
}
